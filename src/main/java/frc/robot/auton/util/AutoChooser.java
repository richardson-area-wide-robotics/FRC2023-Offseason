...
public class AutoChooser {
    ...
    /** 
     * Add an auton to the available auton 
     * This is the sendable chooser 
     * 
     * @param auton Auton command to run.
     * @param name anme of the command as displayed in the drop down sendable list 
     */
    public static void addSendalbeAuton(Command auton, String name){
      // check the give string name to make sure it is not already created 
      if (m_autons.containsKey(name)) {
        System.out.println("Auton Already exists with the name {}!" + name);
        return;
      } 
      ...
      m_autons.put(name, auton);
      m_autoList.add(name);
      autonomousChooser.addOption(name, auton);
    ...
    /**
     * Get the seleced auton selected from drop down sendable chooser, or the default if not found
     * 
     * @return Auton command to run 
     */
    public static Command getSendableAuton(){
      Command selectedAuton = autonomousChooser.getSelected();
